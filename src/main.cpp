...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CanadaEhCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CanadaEhCoin Signed Message:\n";
...
{
    // CanadaEhCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 12 * 60 * 60; // CanadaEhCoin: 0.5 day
static const int64 nTargetSpacing =  60; // CanadaEhCoin: 60 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CanadaEhCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, CanadaEhCoin - DarkGravity v2, written by Evan Duffield - evan@CanadaEhCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // CanadaEhCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // CanadaEhCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
         ...
         // Genesis block        
        const char* pszTimestamp = "July 5, 2014, 1:45 AM EST. A new day, and Canada Eh Coin is born. CEC.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xf6, 0xc6, 0xb6, 0xd6 }; // CanadaEhCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CanadaEhCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CanadaEhCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CanadaEhCoinMiner(CWallet *pwallet)
{
    printf("CanadaEhCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("CanadaEhCoin-miner");
    ...
    try { loop {
        ...
        printf("Running CanadaEhCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CanadaEhCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CanadaEhCoinMiner, pwallet));
}
